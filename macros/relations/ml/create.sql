{% macro snowflake__get_create_forecast_sql(relation, sql, input_data=none, timestamp_colname=none, target_colname=none, series_colname=none, config_object=none, object_tags=none, comment=none) -%}
  ...
  create or replace snowflake.ml.forecast {{ relation }}(
...
{% macro snowflake__get_create_anomaly_detection_sql(relation, sql, input_data=none, timestamp_colname=none, target_colname=none, label_colname=none, series_colname=none, config_object=none, object_tags=none, comment=none) -%}
  {%- set timestamp_colname = config.get('timestamp_colname', default=none) if timestamp_colname is none else timestamp_colname -%}
  {%- set target_colname = config.get('target_colname', default=none) if target_colname is none else target_colname -%}
...
{% macro snowflake__get_create_classification_sql(relation, sql, input_data=none, target_colname=none, config_object=none, object_tags=none, comment=none) -%}
  {%- set target_colname = config.get('target_colname', default=none) if target_colname is none else target_colname -%}
  {%- set config_object = config.get('config_object', default=none) if config_object is none else config_object -%}
  {%- set input_data = sf_ai.input_data(sql, config.get('input_data', default=none) if input_data is none else input_data) -%}
  {%- set object_tags = config.get('object_tags', default={}) if object_tags is none else object_tags -%}
  ...
  create or replace snowflake.ml.classification {{ relation }}(
    input_data => {{ input_data }},
    target_colname => {{ sf_ai.sql_string(target_colname) }}
    {%- if config_object is not none and config_object != '' %},
    config_object => {{ config_object }}
